...
import { createTextarea } from '../Ui/text-area';
import { createRadioORCheckbox } from '../Ui/radioCheckbox';
...
class PreferenceComponent {
  ...
  render(): HTMLDivElement {
    ...
    automatedRebalancingRadios.forEach((radio) => {
      ...
      if (
        this.stateManager.getForm().automatedRebalancing &&
        this.stateManager.getForm().automatedRebalancing === radioLabel
      ) {
    ...
    taxSavingPreferenceRadios.forEach((radio) => {
      ...
      if (
        this.stateManager.getForm().taxSavingPrefernce &&
        this.stateManager.getForm().taxSavingPrefernce === radioLabel
      ) {
        radioInput.checked = true;
      }
    ...
    financialGoalsTextBox.value = this.stateManager.getForm().financialGoals;
    financialGoalsTextBox.addEventListener('input', (event) => {
      this.stateManager.getForm().financialGoals = (event.target as HTMLTextAreaElement).value;
    });
    ...
    if (riskAckCheckbox) {
      riskAckCheckbox.checked = this.stateManager.getForm().riskAcknowledgement;
    }
    riskAckCheckbox.addEventListener('change', (event) => {
      const target = event.target as HTMLInputElement;
      this.stateManager.getForm().riskAcknowledgement = target.checked;
      validatePart3AckCheckBox();
    });
...
const preferenceComponent = new PreferenceComponent(stateManager);
const Preference = (): HTMLDivElement => {
  return preferenceComponent.render();
};
